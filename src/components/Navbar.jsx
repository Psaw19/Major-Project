    ...
    return (
        <div className='w-full shadow-md text-lg md:text-xl font-medium relative'>
            ...
            <div className='flex sm:items-baseline gap-5 justify-between sm:justify-start' >
                ...
                <div ref={navRef} className={`absolute left-0 ${toggle ? 'top-[100%]' : 'top-[-350%]'} transition-all duration-300 flex flex-col items-center justify-center w-full bg-black/30 backdrop-blur py-2 z-50`}>

                    <Link to='/' onClick={() => setToggle(toggle => !toggle)} className='w-full hover:bg-black/10'>
                        <button className='p-3 w-full'>Home</button>
                    </Link>

                    <Link to='/about' onClick={() => setToggle(toggle => !toggle)} className='w-full'>
                        <button className='p-3 w-full hover:bg-black/10'>About</button>
                    </Link>

                    <Link to='/start' onClick={() => setToggle(toggle => !toggle)} className='w-full'>
                        <button className='p-3 w-full hover:bg-black/10'>Start</button>
                    </Link>
